...
select
    ...
    floor(datediff(from_unixtime(o.add_time,'yyyy-MM-dd HH:mm:ss'), concat(year(from_unixtime(o.add_time,'yyyy-MM-dd HH:mm:ss')), '-01-01')) / 7) + 1 week,
...
select 
    ...
    min_stime
from t2_1
group by id,orderinfoid, OrderNumber, year, month, week, day, scn_region, province, city, district, channel, netpoint_id, netpoint_name,
...
insert overwrite table ads_battery_o2o_service_quality partition(ds = ${bdp.system.bizdate})
select
    row_number() over() id, -- 主键ID
    coalesce(t2.year, 0) year,                   -- 年
    coalesce(t2.month, 0) month,                  -- 月
    coalesce(t2.week, 0) week,                   -- 周
    -- day,
    scn_region,             -- scn大区
    t2.province,               -- 省
    t2.city,                   -- 市
    ...
    coalesce(nt.netpoint_type, '睡眠网点') netpoint_type,                                                                 -- 网点类型
    if(datediff(date_add(concat(year, '-01-01'),cast((week-1)*7 as int)),min(min_stime)) <= 90, 1, 0) is_new_netpoint,     -- 是否新网点
    
    case when if(datediff(date_add(concat(year, '-01-01'),cast((week-1)*7 as int)),min(min_stime)) <= 90, 1, 0) = 1 
            and t4.last_3_mon_dispatch_order >= 15 then 1
         when if(datediff(date_add(concat(year, '-01-01'),cast((week-1)*7 as int)),min(min_stime)) <= 90, 1, 0) = 0 
    ...
    coalesce(round(avg((handletime - dispatchtime) / 3600 ), 2), 0) avg_order_time,   -- 平均接单时效
    coalesce(round(avg(if(completetime is not null and completetime > handletime ,(completetime - handletime) / 3600 ,null )), 2), 0) avg_install_time,   -- 平均服务时效
    coalesce(round(count(if(completetime is not null and (completetime > appointtime), 1,null)) / count(distinct id), 2), 0) ontime_service_rate,    -- 及时服务时效
    coalesce(round(avg((appointtime - dispatchtime) / 3600 ), 2), 0) ontime_appointment_time,   -- 平均及时预约率
    
    coalesce(count(if(completetime is not null and ((completetime - addtime) / 3600) < 2, 1, null)), 0) within_2_hour_order_num,     -- 2小时安装时效达成订单数
    coalesce(round(count(if(completetime is not null and (completetime - addtime) / 3600 < 2, 1, null)) / count(distinct id),2), 0) within_2_hour_rate, -- 2小时安装时效达成率
    
    coalesce(count(if(completetime is not null and ((completetime - addtime) / 3600) < 4, 1, null)), 0) within_4_hour_order_num,     -- 4小时安装时效达成订单数
    coalesce(round(count(if(completetime is not null and (completetime - addtime) / 3600 < 4, 1, null)) / count(distinct id),2), 0) within_4_hour_rate, -- 4小时安装时效达成率
    ...
    coalesce(count(if(((handletime - dispatchtime) / 60) <= 10  , 1, null)), 0) within_10_min_order_num,  -- 10分钟接单量
    coalesce(round(count(if(((handletime - dispatchtime) / 60) <= 10  , 1, null)) / count(distinct id), 2) , 0) response_rate,      -- 10分钟响应率
    coalesce(round(count(if(((handletime - dispatchtime) / 60) <= 10  , 1, null)) / count(distinct id), 2) * 100, 0) response_score,    -- 时效分
    ...
    null total_health_score,  -- 综合健康评分
...
left join t3 on t2.week = t3.week and t2.year = t3.year and t3.month = t2.month and t2.netpoint_id = t3.netpoint_id and t3.netpoint_name = t2.netpoint_name
...
group by t2.year,t2.month,t2.week,scn_region, t2.province,t2.city,district, channel, t2.netpoint_id,t2.netpoint_name,address,nt.netpoint_type,t3.result,t3.first_3_mon_dispatch_order,t4.last_3_mon_dispatch_order                           
...
group by t2.year,t2.month,t2.week,scn_region, t2.province,t2.city,district, t2.netpoint_id,t2.netpoint_name,address,nt.netpoint_type,t3.result,t3.first_3_mon_dispatch_order,t4.last_3_mon_dispatch_order                           
