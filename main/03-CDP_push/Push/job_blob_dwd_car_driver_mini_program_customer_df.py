...
class TOBLOB():
    ...
    def hive_data_to_csv(self,spark):
        ...
        df = df.drop('license_plate9')\
               .drop('vin9') \
               .withColumn("phone9", AES_udf(df['phone9'])) \
               .withColumn("nick_name9", AES_udf(df['nick_name9'])) \
               .withColumn("real_name9", AES_udf(df['real_name9'])) \
               .withColumn("sell_name9", AES_udf(df['sell_name9']))
        df.show()
...
if __name__ == '__main__':
    ...
    # assignment
    table_name = 'dwd_car_driver_mini_program_customer_df'
